...
class LearnerController {
    ...
    def showChoice()
    {
        println("show choice params "+params +""+session)
        def cid = params.cid
        CourseOffering courseOffer = CourseOffering.findById(cid)
        println("course_Offer"+courseOffer)
        String uid = session.uid
        Learner student = Learner.findByUid(uid)
        println("Student ....................."+student)
        def studentLearner = CourseOfferingLearner.findByLearnerAndCourseoffering(student,courseOffer)
        println("studentLearner.............."+studentLearner)
        MCQExam mcqData = MCQExam.findById(params.id)
        println("ka error ........")
        ...
        def mcqsubdata = MCQExamSubmission.findByCourseofferinglearnerAndMcqexam(studentLearner,mcqData)
        println("vlue........"+mcqsubdata)
    ...
    def generateQuestion()
    {
        ...
        println("student"+student)
        def studentLearner = CourseOfferingLearner.findByLearnerAndCourseoffering(student,courseOffer)
        def allMcqSubData = MCQExamSubmission.findAllByCourseofferinglearner(studentLearner)
        ...
        def mcqop = mcqData.mcqexam
        def optionsize = MCQExamOptions.findAllByMcqexam(mcqop) as List
        println "Options" + optionsize + "size" + optionsize.size()
        def opsize = optionsize.size()
        def mcqsubdata = MCQExamSubmission.findByCourseofferinglearnerAndMcqexam(studentLearner,mcqData.mcqexam)
        println("vlue........"+mcqsubdata)
        [mcqData:mcqData,size:size.size,cid:cid,mcqpatern:mcqpatern,opsize:opsize,allMcqSubData:allMcqSubData,mcqsubdata:mcqsubdata]
    }
    def generateQues()
    {
        ...
        def mcqsubdata = MCQExamSubmission.findByCourseofferinglearnerAndMcqexam(studentLearner,mcqData.mcqexam)
        println("vlue........"+mcqsubdata)
        [mcqData:mcqData,size:size.size,qno:qno,cid:cid,mcqpatern:mcqpatern,opsize:opsize,allMcqSubData:allMcqSubData,mcqsubdata:mcqsubdata]
    }
    def saveQuestionData()
    {
        ...
        println(opList.size())
        if(opList.size()>0) {
            def prevData = MCQExamSubmission.get(mcqesub.id)
            if(prevData) {
                prevData.mcqexamoption.clear()
            }
        ...
        if(session.counter<size.size) {
            ...
            println "Options" + optionsize + "size" + optionsize.size()
            def opsize = optionsize.size()
            [mcqData: mcqData, size: size.size, cid: courseOffer.id, mcqpatern: mcqpatern,opsize:opsize,allMcqSubData:allMcqSubData]
        }
        else
        {
            println "end exam course parameter"+courseOffer
            println "end exam studentLearner" + studentLearner
            render"<button class='btn btn-success' onclick='calculateMarks'>Submit</button>"
        }
    }
    def calculateMarks()
    {
