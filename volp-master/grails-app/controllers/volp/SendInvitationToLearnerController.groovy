...
class SendInvitationToLearnerController {
    ...
    def comparecodejoinclass()
    {
        ...
        SendInvitationToLearner sitl=SendInvitationToLearner.findByEmailAndCode(email,secretcode)
        if(sitl!=null)
        {
            println("Joined course::"+sitl.courseoffering.course.course_name)
            CourseOfferingLearner cofflearner=CourseOfferingLearner.findByCourseofferingAndLearner(sitl.courseoffering,l)
            if(cofflearner==null)
            {
