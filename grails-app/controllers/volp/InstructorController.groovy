...
class InstructorController {
    ...
    def instructor(){
        ...
        println("Ins:"+ins)
        def instructorcourses=Course.findAllByUsername(session.uid)
...
//        println("pathimg:"+pathimg)
    ...
    def addCourse(){
        Instructor ins = Instructor.findByUid(session.uid)
        session.msg = ""
        def crs_cat = CourseCategory.list()
...
//saving
    ...
    def profile()
    {
        ...
        [username:username,genderInfo:genderInfo,insData:insData,role:role,contactInfo: contactInfo,addressInfo:addressInfo,yearInfo:yearInfo]
    ...
    def storeInstructorData()
    {
        ...
        if(userInfo.usertype.type == "Instructor")
        {
            ...
            if(!insData)
            {
                ...
                //def fp=FolderPath.list()+"/profile"
                def file = request.getFile("profilePic")

                String image_path=fp[0].path+"/profile/"
