    ...
    def saveCourseFee() {
        ...
        if (params.insCourses.empty | params.courseFeeType.empty | params.CertificateType.empty) {
            ...
            if (!crsf) {
                ...
                /*  println("not null")
                  println("crsf" + crsf)
                  def crft = crsf.coursefeestype
                  println("crft" + crft + "crsffety" + crsffety)
                  def a = CourseFees.findAllByCourseoffering(crsOff)
                  def data = (a.coursefeestype.id).flatten()
                  println(data)
                  int i = 0
                  int count = 0
                  ...
                  println(data.size)
                  for (; i < data.size; i++) {
                      if (data[i] == Integer.parseInt(params.courseFeeType)) {
                          println("math")
                          count = count + 1
                      }
                  }
                  if (count >= 2) {
                      println("i am in flag" + count)
                      flag = 0
                  } else {
                      flag = 1
                  }
                  println(count + "fffff" + flag)*/
                if (flag) {
                    CourseFees ob = new CourseFees();
                    ob.username = session.uid
                    ob.fees = Double.parseDouble(params.Doller)
